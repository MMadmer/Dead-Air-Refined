...
| `item.spawn` | `section`, `count`, `items: item_list`, **`where: loot_spawn`**, and per row: `condition`, `ammo`, `ammo_kind`, `uses`, `upgrades`, `ref` | Creates items. What: `section`+`count`, or an `items` list of section rows - and **every row carries its own state**, so two rifles and a medkit in one list do not share a condition. The one-line spelling is one row, so the same keys are legal beside `section`; beside `items` they belong to no row and are refused. Where: one `loot_spawn` form - `place` (several instances spread over its radius), `into`, `stash`, `npc`, `corpse`, `new_corpse` - exactly what `objective.fetch` resolves. Row state: `condition` and `upgrades` are written on the server entity, `ammo` is the magazine of a weapon (`set_ammo_elapsed`) or the content of one box (`create_ammo`, clamped to `box_size`), `ammo_kind` is the cartridge, resolved to its index in the weapon's `ammo_class`. `uses` is the one field with no server side (`CEatableItem` keeps it in the client packet) and waits as an **online job** (§5). A field that means nothing for its section is skipped with a log line, not applied blindly. An upgrade id the section cannot install is skipped, and a duplicate is dropped before `add_upgrade` - the engine `FATAL`s on one and no `pcall` catches that. `ref` names one object, so it needs a row with count 1. |
...
state = {
  ...
  quests = {
    ["madmer.cordon_tales.wolf_debt"] = {
      ...
      jobs   = { [5124] = { uses = 0 } },   -- online jobs: applied by the poll once the client object exists (kill / uses)
    },
  },
}
```
