...
| `objective.fetch` | `section: item_section`, `count: int = 1`, `items: item_list`, `from: object_ref`, `spawn: loot_spawn`, `map_spot: bool = true`, `spot_text: text` | `done` | ✔ | `false` | Waits for items to reach the player. What to bring is the `items` list; **a row is either a section with a count, or one concrete object** (`{ item = { ref/story } }`) — the two mix freely, so “bring THE flash drive and three medkits” is one node. `section`+`count` is the one-line spelling of a one-row list. With `from` only items that came **out of that container** count (section rows; an identical item picked up elsewhere never moves the counter). With `spawn` the node **creates the loot itself on entry** and credits exactly those instances — a destroyed instance is recreated (in its host, or at the player's feet when the host is gone) so the quest cannot dead-end; concrete-object rows cannot be combined with `spawn`. `map_spot` keeps a marker on the source — the host container/corpse/NPC, each loose spawned item, and whatever carries a concrete-object row right now. `count` is whole items — for ammo that means boxes. |
