...
| `item_list` | `{ { section = "bread", count = 2 }, { section = "medkit" }, … }` | several kinds at once; a section may appear once |
...
| `objective.fetch` | `section: item_section`, `count: int = 1`, `items: item_list`, `item: object_ref`, `from: object_ref`, `spawn: loot_spawn`, `map_spot: bool = true`, `spot_text: text` | `done` | ✔ | `false` | Waits for items to reach the player. Exactly one of three forms (`E022`): **`section`** (+`count`), **`items`** (several sections, each with its own count), or **`item`** (one concrete object by `ref`/`story`). Plain forms count anything of the section in the inventory. With `from` only items that came **out of that container** count — an identical item picked up elsewhere never moves the counter. With `spawn` the node **creates the loot itself on entry** and credits exactly those instances — same protection, plus a destroyed instance is recreated (in its host, or at the player's feet when the host is gone) so the quest cannot dead-end. `map_spot` keeps a marker on the source (host container/corpse/NPC, or each loose item) while the node waits. `count` is whole items — for ammo that means boxes. |
