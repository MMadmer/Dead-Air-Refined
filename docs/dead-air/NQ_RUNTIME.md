...
| `npc_list` | `{ { character = "valera", ref = "valera" }, { section = "sim_default_stalker_1", unique = true } }` | the rows of `spawn.npc`, one NPC each: exactly one of `character` \| `section`, plus optional `ref`, `unique`, `dead`, `yaw`. A key outside that set fails the load - a typo must not become a silent no-op. `character` is the id of a character the module's editor project declares; the section is `<module>_<id>`, the module taken from the graph's definition (a module id may carry dots of its own). `"<other module>.<id>"` reaches a character another module ships |
...
| `objective.kill` | **`target: kill_target`**, `by_actor: bool = false`, `spot_task: task_id`, `spot_objective: objective_id`, `spot_done: bool = true` | `done` | ✔ | `false` | Waits for the target to die: an NPC by story id, an object remembered under a `ref`, or a squad from `spawn` (created on entry and remembered under its `ref`). `by_actor` additionally demands that the player landed the kill. With `spot_task` the node re-points the bound task's own PDA marker (or the marker of the step named by `spot_objective`) at the target - the task's colour, no separate additive spot. Nothing is removed when the node ends: completing the task/step takes the marker off the map, exactly like after a manual `task.set_target`. For a squad the quest just spawned this is the only way the player can find it. With `spot_done` (default **on**) the node also **closes what it borrowed** when it completes: the step named by `spot_objective`, or the whole task when no step is named. Turn it off (`spot_done = false`) when several nodes feed one step and only the last one should end it. It fires before the node's own `on_exit`, only on the `done` pin and never on a cancel, and it never closes a step somebody already closed. |
...
| `objective.fetch` | `section: item_section`, `count: int = 1`, `items: item_list`, `from: object_ref`, `spawn: loot_spawn`, `spot_task: task_id`, `spot_objective: objective_id`, `spot_done: bool = true` | `done` | ✔ | `false` | Waits for items to reach the player. What to bring is the `items` list; **a row is either a section with a count, or one concrete object** (`{ item = { ref/story } }`) — the two mix freely, so “bring THE flash drive and three medkits” is one node. `section`+`count` is the one-line spelling of a one-row list. With `from` only items that came **out of that container** count (section rows; an identical item picked up elsewhere never moves the counter). With `spawn` the node **creates the loot itself on entry** and credits exactly those instances — a destroyed instance is recreated (in its host, or at the player's feet when the host is gone) so the quest cannot dead-end; concrete-object rows cannot be combined with `spawn`. `spot_task` re-points the bound task's/step's marker at the **nearest pending** source — the host container/corpse/NPC, a loose spawned item, or whatever carries a concrete-object row right now; collected targets drop out and the marker walks on by itself. `count` is whole items — for ammo that means boxes. With `spot_done` (default **on**) the node also **closes what it borrowed** when it completes: the step named by `spot_objective`, or the whole task when no step is named. Turn it off (`spot_done = false`) when several nodes feed one step and only the last one should end it. It fires before the node's own `on_exit`, only on the `done` pin and never on a cancel, and it never closes a step somebody already closed. |
| `objective.reach` | **`place: place`**, `spot_task: task_id`, `spot_objective: objective_id`, `spot_done: bool = true` | `done` | ✔ | `false` | Waits until the player is inside the place. With `spot_task` the bound task's/step's marker is pointed at it (at an anchor restrictor for a bare position); when the node releases its own anchor, that one marker is cleared rather than left on a dead id. With `spot_done` (default **on**) the node also **closes what it borrowed** when it completes: the step named by `spot_objective`, or the whole task when no step is named. Turn it off (`spot_done = false`) when several nodes feed one step and only the last one should end it. It fires before the node's own `on_exit`, only on the `done` pin and never on a cancel, and it never closes a step somebody already closed. |
...
|---|---|---|
| `item.give` | `section: item_section`, `count: int = 1`, or `items: item_list` | Gives items to the player: one section with a count, or a list of `{ section, count }` rows (the row schema is `rows`/`row.*` in the catalog). Inside a dialog the transfer goes through the NPC so the talk window shows it; outside, `alife():create` on the actor with a relocate news line. The two spellings are exclusive - the loader refuses both at once and neither (`E022`). |
| `item.take` | `section: item_section`, `count: count_or_all = 1`, or `items: item_list` | Takes items away: one section, or a list of rows; a row's `count` may be `"all"`. Inside a dialog it goes through the NPC. The two spellings are exclusive (`E022`). |
| `item.spawn` | `section`, `count`, `items: item_list`, **`where: loot_spawn`**, and per row: `condition`, `ammo`, `ammo_kind`, `upgrades`, `ref` | Creates items. What: `section`+`count`, or an `items` list of section rows - and **every row carries its own state**, so two rifles and a medkit in one list do not share a condition. The one-line spelling is one row, so the same keys are legal beside `section`; beside `items` they belong to no row and are refused. Where: one `loot_spawn` form - `place` (several instances spread over its radius), `into`, `stash`, `npc`, `corpse`, `new_corpse` - exactly what `objective.fetch` resolves. **Unset means full**: an unwritten `condition` is a new item (the engine's own default) and an unwritten `ammo` fills the magazine, because a server entity starts at zero rounds and a quest reward that comes out empty reads as a bug. `ammo` on an ammo section is the box content (`create_ammo`, clamped to `box_size`); `ammo_kind` is the cartridge, resolved to its index in the weapon's `ammo_class`. Everything is written on the server entity, so a stash nobody opens still holds it. An upgrade id the section cannot install is skipped and a duplicate is dropped before `add_upgrade` - the engine `FATAL`s on one and no `pcall` catches that. A field that means nothing for its section is skipped with a log line. `ref` names one object, so it needs a row with count 1. |
...
| `spawn.npc` | **`npcs: npc_list`**, **`place: place`**, `spread: float = 1.5` | Creates one NPC per row. With several rows each gets a point on a ring of `spread` metres around the place, snapped to the navmesh (on the player's level only - elsewhere there is no navmesh to ask and the simulation places offline objects itself). `unique` removes every NPC of the row's section first: one `alife_scan`, every victim through `prepare_removal` and the safe release, a squad it was alone in removed with it, and every ref in EVERY graph that named it cleared. Its default follows the row - **on for a `character`** (one person: a second Valera is a bug), **off for a `section`** (a kind half the zone may share; default-on would wipe `sim_default_stalker_1` off the map). The kind is cleared once per node run, so two rows of one character are two on purpose. `yaw` / `dead` as in `spawn.object`; the row's `ref` is set to `{ id, kind = "object", section }`. Nothing created = an error that names the section, because the usual cause is a character that was never built into the module. |
...
- **`ref`** — a name the quest gave to something it created itself
  (`spawn.squad`, `spawn.object`, every row of `spawn.npc`, `item.spawn`, `objective.kill { spawn = … }`,
...
state = {
  ...
  quests = {
    ["madmer.cordon_tales.wolf_debt"] = {
      ...
      jobs   = { [5124] = { kill = true } },  -- online jobs: applied by the poll once the client object exists
    },
  },
}
```
