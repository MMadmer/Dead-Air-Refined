...
| `objective.kill` | **`target: kill_target`**, `by_actor: bool = false`, `spot_task: task_id`, `spot_objective: objective_id` | `done` | ✔ | `false` | Waits for the target to die: an NPC by story id, an object remembered under a `ref`, or a squad from `spawn` (created on entry and remembered under its `ref`). `by_actor` additionally demands that the player landed the kill. With `spot_task` the node re-points the bound task's own PDA marker (or the marker of the step named by `spot_objective`) at the target - the task's colour, no separate additive spot. Nothing is removed when the node ends: completing the task/step takes the marker off the map, exactly like after a manual `task.set_target`. For a squad the quest just spawned this is the only way the player can find it. |
