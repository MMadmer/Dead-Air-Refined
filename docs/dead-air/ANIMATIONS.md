...
- **Backpack**: the bag stays in the hands for as long as the inventory is open - drawn on the
  way in, held there while the window is up, put away when it closes. The bag model is the one
  the player wears (`da_backpack_<section>_hud`, else the common one). Pressing the key again
  mid-scene turns the hands around from the point they reached rather than from the first frame,
  so hammering the key cannot make the animation jump. "Game: instant inventory"
  (`dar_instant_inventory`, off by default) shows the window the moment the key is pressed and
  lets the scene run behind it; the bag itself is unaffected either way.
...
| Scenes | `player_hud::scene_play/scene_stop/scene_hold/scene_active/scene_motion_length` | A hand cycle from any hud section with an optional item model in the hand (`item_visual`, `item_position/orientation/scale`, `item_attached`, `item_root_lock`, `lh_lead_gun`) and no `CHudItem` behind it. The cycle can be stretched to the scene length. A scene also ends by time, so a script that forgets to stop it cannot leave the item in the hands. Sections whose cycle is missing from the hands model are logged and skipped, never asserted. |
| Scene start offset | `PlayHudCycle`, `scene_play(..., start_ms)` | A cycle can be started part way in (`CBlend::timeCurrent`), and the scene clock counts only what is left of it. This is what lets an interrupted scene pick its opposite up at the matching pose: half a draw becomes half a put-away, not a whole one from the first frame. |
| Held scene | `player_hud::scene_hold` | Clears the scene clock, so a scene stands until the script stops it. For a scene that has to last as long as a window is open - the backpack while the inventory is up - there is no length to give. `scene_active` is exported so a script can tell that a held scene survived into a fresh game session, where the script's own state did not. |
...
| Input gate | `Level_input.cpp`, `game.only_allow_movekeys` | While a scene runs, only movement, look, pause, console, screenshot, quit and quick save/load presses pass. Releases always pass. The skinning scene adds `level.disable_input()`: its camera is scripted, so movement and look are taken too. The backpack scene deliberately does NOT use the gate: the inventory key is above `kCAM_1` and the gate would swallow the second press the scene needs to reverse. A weapon cannot be drawn during it anyway - `hide_weapon` sets `INV_STATE_BLOCK_ALL`, and `play_blend` drops non-scene hand cycles while a two-hand scene owns the hands. |
...
| `da_backpack_anim` states | `anm_ea_show` while the bag comes out, `anm_ea_idle` held by `game.hold_hud_motion` while the inventory is open, `anm_ea_hide` on the close. The window closing is read from `level.main_input_receiver()`, with a short grace after the script asks for the window: it does not appear in the same frame, and "closed" in those milliseconds would mean the player shut it. A press during either moving cycle starts the opposite one at `1 - progress`. |
