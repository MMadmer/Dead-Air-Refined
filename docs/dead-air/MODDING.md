...
- The active mode set comes from the new-game screen: ticked
  `[character_creation] new_game_<x>_mode` keys are published as XMS modes when
  a new game starts (`new_game_metro_mode` -> `metro`), and a loaded save
  restores the set recorded in its `.scov`. A module adding its OWN mode has to
  put its checkbox on that screen itself - see below.
- Two module scripts run on their own at engine start, in load order, right
  after the `xms` API exists (`xms.load_registrars`): `scripts/mode_register.script`
  (generated by XFined Editor, rewritten on every export) and then
  `scripts/register.script` (the author's, never touched by any tool). Every
  other module script waits for an `xms.require`.
...
| file | what it does |
| --- | --- |
| `patch/<id>_mode.xmlp` | appends `check_<id>_mode` + `cap_check_<id>_mode` to `main_dialog`, grows the frame around the mode column and pushes whatever sat below it down |
| `gamedata/configs/text/rus/<id>_mode.xml` | defines `st_cap_check_<id>_mode`, cp1251 like every other string table |
| `scripts/mode_register.script` | wraps `faction_ui:InitControls` to create the control and `faction_ui:OnStartGame` to write `[character_creation] new_game_<id>_mode` |
