...
| Trunk and crown as one | Outdoor save, `wind_force 1.0`, a tall pine | Pass only when the trunk leans with the crown (no card sliding off it), tips travel more than the trunk, the shadow shows the same lean, and a shot through a bush shakes the branches at the trace |
