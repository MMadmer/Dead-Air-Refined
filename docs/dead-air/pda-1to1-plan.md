...
**Кэши шейдеров.** Любая правка `models_pda.s`, `model_pda_screen.ps` или читаемой ими константы: снести `appdata\shaders_cache_oxr` и `appdata\shaders_cache`. `Run-RenderQa.ps1:135-142` специально засевает копии в QA-рантайм — если снесли настоящие, пересейте, иначе первые числа систематически пессимистичны.
