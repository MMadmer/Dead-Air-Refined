...
begin
  ...
  begin
    Result :=
      ...
      'Нужна корневая папка с xrEngine.exe, fsgame.ltx и database\configs.xdb0.';
    exit;
  end;
  ...
  begin
    Result :=
      ...
      'Сначала удали её старым Uninstall-DeadAir-x64.ps1, затем запусти этот мастер снова.';
    exit;
  end;

  if IsOwnInstallation(DirectoryName, 'standalone') then
  begin
    Result := 'Эта папка уже используется самостоятельной установкой. Выбери режим самостоятельной установки.';
    exit;
  end;
...
begin
  ...
  begin
    Result := 'Эта папка используется обновлённой x86-установкой. Выбери режим обновления существующей игры.';
    exit;
  end;
  ...
  begin
    Result :=
      'Для самостоятельной версии нужна новая или пустая папка.' + #13#10 +
      'Установщик не будет смешивать чистую Dead Air 0.98b с чужими файлами.';
  end;
end;
...
begin
  ModePage := CreateInputOptionPage(
    ...
    'Выбери один из двух вариантов. Содержимое существующей игры и её сохранения в режиме обновления не изменяются.',
    True,
    False);
