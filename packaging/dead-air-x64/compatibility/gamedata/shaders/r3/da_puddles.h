...
da_puddle_result da_puddles(float3 pos_v, float hemi_in, float3 N_in)
{
	...
	// Только горизонтальное. Наклон берётся из ИНТЕРПОЛИРОВАННОЙ вершинной нормали (N_in из
	// G-буфера) - она гладкая по определению. Геометрическая ddx/ddy-нормаль константна
	// внутри треугольника и скачет на его границе: с ней маска (а через маску - глянец)
	// распадалась на пересвеченные полигоны; попытка перенести её в порог сделала хуже -
	// прямые треугольные края «сквозь террейн». gn_w остаётся ниже только для НАПРАВЛЕНИЯ
	// стока ряби (там скачок направления между фасетками глазом не читается).
	const float3 Nn = normalize(N_in);
	const float3 n_w = mul(transpose(V_rot), Nn);
	const float slope = saturate((abs(n_w.y) - 0.80f) * 6.0f);
