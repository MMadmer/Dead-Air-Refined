...
// Ветер погоды для дрейфа ряби: xy — мировое XZ-направление, умноженное на скорость
// (норма 20 м/с, как у дождя), z — порывность (wind_strength_factor). Биндер cl_da_puddle_wind.
...
da_puddle_result da_puddles(float3 pos_v, float hemi_in, float3 N_in)
{
	...
	// Ветровое состояние: скорость (длина xy) с весом порывности. Управляет и амплитудой волнения,
	// и сносом ряби ниже.
	const float wind_k = saturate(length(da_puddle_wind.xy)) * (0.4f + 0.6f * da_puddle_wind.z);
	// Нижняя граница волнения теперь ЖИВАЯ: в настоящий штиль без дождя вода почти зеркало (0.12),
	...
	{
		// ⚠️ Было: сдвиг узора по синусам времени — рисунок ездил по эллипсу туда-обратно, и на
		// стоячей воде это читалось как «качающаяся статика», а на луже через бугор — как течение,
		// которое ходит в гору и обратно. Прибавлять timers.x напрямую по-прежнему нельзя (frac()
		// шума теряет точность на больших числах — «хожу по озеру»), поэтому дрейф собран как у
		// flowmap-водопадов: ДВА слоя с полупериодным сдвигом фазы, каждый ползёт от своего начала
		// цикла и получает нулевой вес ровно в момент своего сброса. Смещение внутри цикла
		// ограничено, точность не деградирует, а глазу слои сливаются в непрерывный поток.
		//
		// Куда и с какой скоростью плыть — из физики:
		//   * стекание по склону, только пока идёт дождь (стоячая лужа на уклоне не течёт):
		//     горизонтальная компонента геометрической нормали указывает в сторону спуска, её
		//     длина — крутизна;
		...
		const float2 q1 = wp * 12.0f + drift * (f1 * T);
		const float2 q2 = wp * 12.0f + drift * (f2 * T) + 37.7f; // свой рисунок у второго слоя
		...
		const float h1 = da_vnoise(q1);
		const float2 g1 = float2(da_vnoise(q1 + float2(0.35f, 0.0f)) - h1,
		                         da_vnoise(q1 + float2(0.0f, 0.35f)) - h1);
		const float h2 = da_vnoise(q2);
		const float2 g2 = float2(da_vnoise(q2 + float2(0.35f, 0.0f)) - h2,
		                         da_vnoise(q2 + float2(0.0f, 0.35f)) - h2);
		// При нулевом дрейфе (штиль, ровное дно) кроссфейд двух статичных узоров остаётся мягким
		// «дыханием» воды — ровно то немногое, что делает стоячая лужа.
		ripple = (g1 * w1 + g2 * w2) * (0.18f * rain_now * near_f * da_puddle_look.w);
	}
