#include "common.h"
#include "tree_instance.h"

uniform float3x4 m_xform;
uniform float3x4 m_xform_v;
uniform float4 consts;
uniform float4 c_scale, c_bias, wind, wave;

// Needs the wind/wave uniforms above, so it comes after them.
#include "tree_wind.h"

#ifdef USE_AREF
v2p_shadow_direct_aref main(v_shadow_direct_aref I, uint instance_id : SV_InstanceID)
#else
v2p_shadow_direct main(v_shadow_direct I, uint instance_id : SV_InstanceID)
#endif
{
#ifdef USE_AREF
    v2p_shadow_direct_aref O;
#else
    v2p_shadow_direct O;
#endif

    float3x4 local_xform = m_xform;
    if (tree_instance_control.x > 0.5f)
        local_xform = tree_instance_xform(instance_id);

    float3 pos = mul(local_xform, I.P);
    float base = local_xform._24;
    float dp = calc_cyclic(wave.w + dot(pos, (float3)wave));
    float H = pos.y - base;
    float inten = H * dp;
    float2 result;
    float3 extra;
#ifdef USE_TREEWAVE
    result = 0;
    extra = 0;
#else
#ifdef USE_AREF
    float frac = I.tc.z * consts.x;
#else
    float frac = 0;
#endif
    result = calc_xz_wave(wind.xz * inten, frac);
    // Branch sway only, so the shadow follows the crown it belongs to. The leaf flutter is
    // deliberately absent here (zero normal skips it): a few centimetres are sub-texel in
    // the shadow map and would only shimmer the shaded pixels.
    extra = tree_wind_extra(I.P.xyz, H, float3(0, 0, 0), frac);
#endif

    float4 f_pos = float4(pos.x + result.x + extra.x, pos.y + extra.y, pos.z + result.y + extra.z, 1);
    O.hpos = mul(m_VP, f_pos);
#ifdef USE_AREF
    O.tc0 = (I.tc * consts).xy;
#endif
#ifndef USE_HWSMAP
    O.depth = O.hpos.z;
#endif
    return O;
}
FXVS;
