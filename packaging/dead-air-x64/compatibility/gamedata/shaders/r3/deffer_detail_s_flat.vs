...
v2p_flat 	main (v_detail v, uint instance_id : SV_InstanceID)
{
	...
	// Wind motors: still details do not wave, but a boot or a blast still bends them.
	{
		...
		// Cap short of lying flat - excess bend would pure-stretch the blade, and a tip on
		// the exact ground plane buries itself on slopes (see w_flat).
		mb *= min(1.0f, 0.92f * H_s / max(length(mb), 0.001f));
