...
//	xyz = extinction sigma_t per channel (1/m), w = reserved (phase g).
...
float2 da_w_detail(Texture2D nmap, SamplerState smp, float2 wxz, float2 wdir, float t, float ms)
{
	...
	const float2 wd1 = float2(wdir.x * 0.8763f - wdir.y * 0.4818f, wdir.x * 0.4818f + wdir.y * 0.8763f);
	...
	const float2 uv0 = f0 * float2(1.35f, 1.90f) - float2(drift, 0.0f);
	const float2 uv1 = f1 * float2(2.60f, 3.30f) - float2(drift * 1.7f, drift * 0.31f) + 0.37f;
...
{
	const float u = max(ms * gust, 0.0f);
	const float clean = 0.003f + 5.12e-3f * u;
	const float slick = 0.008f + 1.56e-3f * u;
	return max(lerp(clean, slick, saturate(scum)), 1e-4f);
}
