...
cbuffer DaFireRender
{
	...
	float4	da_fr_c;	// x = smoke gain, y = detail erosion, z = time, w = crossover with the sprite-free flame
}
...
void DaSample(float weight, float3 O, inout float3 radiance, inout float trans)
{
	...
	const float3 emission = da_fr_ramp(T) * (Tn * Tn * Tn) * soot * da_fr_a.x;

	//	Only the soot that has cooled swallows light. Inside the flame the same soot is the
	//	thing doing the glowing, and a plume that absorbed there would hide the fire behind
	//	its own smoke - which is exactly what a flame does not do.
	const float cool = saturate(1.0 - T * 1.2);
	const float sigma = da_fr_a.y * smoke * cool;
	const float3 scatter = (DiffuseLight.rgb + da_fr_b.rgb * saturate(T * 2.0 + 0.15 * s.w))
		* (da_fr_a.z * smoke * cool);
...
float4 DaRaycast(PS_INPUT_RAYCAST input)
{
	...
	//	is there so a ray straight up the core cannot turn into an aliasing white hole.
	return float4(min(radiance, 24.0) * da_fr_c.w, (1.0 - trans) * da_fr_c.w);
}
