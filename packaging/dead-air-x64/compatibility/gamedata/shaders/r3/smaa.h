#ifndef SMAA_H_INCLUDED
#define SMAA_H_INCLUDED

// SMAA 1x library (v2.8, github.com/iryoku/smaa), ported from IX-Ray 1.6 stcop
// (gamedata\shaders\d3d11\smaa.hlsli) into the Dead Air r3 shader dialect.
//
// Environment prologue below replaces the donor's per-file setup: common.h brings the
// engine samplers (smp_rtlinear / smp_nofilter) and s_image; SM_5 / SM_4_1 come from the
// engine's shader-option list (r4_shaders.cpp); screen_res is the engine constant
// (W, H, 1/W, 1/H) bound by name (Blender_Recorder_StandartBinding.cpp), so
// SMAA_RT_METRICS - which SMAA wants as (1/W, 1/H, W, H) - is its .zwxy swizzle.
// The donor used the identical swizzle on its scaled_screen_res.
...
uniform float4 screen_res; // x=W, y=H, z=1/W, w=1/H
...
// Porting Functions
    ...
    // Engine samplers from common_samplers.h: rtlinear = linear/clamp,
    // nofilter = point/clamp - exactly the Linear/PointSampler contract SMAA needs.
...
#if !defined(SMAA_HLSL_4) && !defined(SMAA_HLSL_4_1)
    #error smaa.h expects the r3/DX11 environment (SMAA_HLSL_4 or SMAA_HLSL_4_1)
#endif
...
// Vertex Shaders
//
// In this port they are called from the PIXEL shaders with the interpolated
// texcoord (the donor's own R2 path does the same): every output is an affine
// function of texcoord, so per-pixel evaluation is bit-identical to
// interpolating VS outputs.
