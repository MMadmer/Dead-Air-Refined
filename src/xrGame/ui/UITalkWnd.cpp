...
void CUITalkWnd::AddQuestion(const shared_str& text, const shared_str& value, int number, bool b_finalizer)
{
    ...
    // Shown as written. Dead Air 1.0 garbled the phrases by psy health, and 1.2.0 carried that
    // over - but 0.98b, the game this runs, never did it (AddQuestion/AddAnswer at 0x104CAC30 and
    // 0x104CAB70 translate and pass through), and there psy health drains by design
    // (psy_health_v < 0, six times faster under Revolution II) with nothing on screen saying so.
    // A few hours into any game the dialogues went to ?, * and ~, and read as a broken font.
