...
void CSnork::Load(LPCSTR section)
{
    ...
    // Some addon models provide the original transition cycle, while the shipped snork model does not.
    if (anim().AddAnim(eAnimStandLieDown, "stand_lie_down_", -1, &velocity_none, PS_STAND, fxs, false))
    {
