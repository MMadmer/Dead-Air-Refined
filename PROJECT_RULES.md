...
- Перед любой правкой сначала сравнить затронутый код и поведение с оригинальной
  ...
  скрипту или шейдеру. Для багов эта проверка обязательна до выбора причины и
  способа исправления.
- Если x64-реализация расходится с оригиналом, сначала восстановить паритет и
  считать оригинальное поведение эталонным. Собственное решение допускается
  только когда задача явно требует изменить или исправить поведение самого
  оригинала либо когда доказано, что дефект присутствует и в оригинале.
...
- Текст релиза писать на английском языке, коротко и только по подтверждённым
...
- Перед публикацией сверить номер версии, имена обоих assets и текст с фактически
  собранными пакетами.
